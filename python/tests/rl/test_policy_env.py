...
def ejecutar(env: PedemonteTemporalV4PlanEnv, secuencia: tuple[str, ...]) -> tuple[float, dict[str, Any]]:
...
class RLTemporalV4EnvTest(unittest.TestCase):
    ...
    def crear_env(self) -> PedemonteTemporalV4PlanEnv:
        return PedemonteTemporalV4PlanEnv(self.instancia, proveedor_viaje=self.proveedor, configuracion_reward=self.reward)

    def test_dimensiones_separadas_historico_y_v4(self) -> None:
        historico = PedemontePlanEnv(self.instancia, proveedor_viaje=self.proveedor)
        v4 = self.crear_env()
        try:
