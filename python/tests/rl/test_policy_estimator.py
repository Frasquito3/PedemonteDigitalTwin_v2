...
from planner.rl.policy_estimator import calcular_arrepentimiento_local_v4, calcular_reward_terminal_v4, proyectar_consecuencias_segundo_orden_v4
...
class TemporalV4EstimatorTest(unittest.TestCase):
    ...
    def test_arrepentimiento_prefiere_este(self) -> None:
        consecuencias = proyectar_consecuencias_segundo_orden_v4(self.instancia, self.matriz, self.configuracion, ('B04-NORTE-TEMPRANO',))
        resultado_este = calcular_arrepentimiento_local_v4(consecuencias, 'B04-ESTE-MEDIO', self.temporal)
        resultado_cercana = calcular_arrepentimiento_local_v4(consecuencias, 'B04-CERCANA-TARDE', self.temporal)
    ...
    def test_banda_terminal_factible_domina_costo_extremo(self) -> None:
        ...
        factible = calcular_reward_terminal_v4(resumen_factible, reward_costo_base=-100.0, configuracion=self.temporal)
        riesgoso = calcular_reward_terminal_v4(resumen_riesgoso, reward_costo_base=100.0, configuracion=self.temporal)
