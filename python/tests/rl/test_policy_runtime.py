...
import sys
import pytest
from planner.rl.policy_runtime import ConfiguracionOperacionRL, FUENTE_RL_UNICA, RLPolicyOperationalPlanner, VERSION_OPERACIONAL, cargar_configuracion_operacional
...
def _config(tmp_path: Path) -> ConfiguracionOperacionRL:
    return ConfiguracionOperacionRL(version=VERSION_OPERACIONAL, modelo=tmp_path / 'rl_policy.zip', sha256_modelo='', max_pedidos_rl_validado=12, usar_mascara_temporal_dura=True)
...
def test_ejecuta_una_sola_politica_y_registra_decision(monkeypatch, tmp_path):
    ...
    planner = RLPolicyOperationalPlanner(configuracion_operacional=_config(tmp_path), planner_rl=planner_falso)
...
def test_fallo_de_politica_informa_error_ejecutable(monkeypatch, tmp_path):
    ...
    planner = RLPolicyOperationalPlanner(configuracion_operacional=_config(tmp_path), planner_rl=_PlannerFalso(error=RuntimeError('fallo controlado')))
...
def test_carga_manifiesto_unico_y_valida_hash(tmp_path):
    ...
    manifest.write_text(dumps({'version': VERSION_OPERACIONAL, 'modelo': modelo.name, 'sha256_modelo': sha256(b'single-policy').hexdigest(), 'max_pedidos_rl_validado': 12, 'usar_mascara_temporal_dura': True}), encoding='utf-8')
...
def test_rechaza_hash_incorrecto(tmp_path):
    ...
    modelo.write_bytes(b'single-policy')
    manifest = tmp_path / 'rl_policies.json'
    manifest.write_text(dumps({'version': VERSION_OPERACIONAL, 'modelo': modelo.name, 'sha256_modelo': '0' * 64}), encoding='utf-8')
...
def test_construye_planner_con_mascara_dura(monkeypatch, tmp_path):
    ...
    config = ConfiguracionOperacionRL(version=VERSION_OPERACIONAL, modelo=modelo, sha256_modelo='', max_pedidos_rl_validado=12, usar_mascara_temporal_dura=True)
    capturado = {}
    ...
    module.RLTemporalV4Planner = PlannerConstruido
    monkeypatch.setitem(sys.modules, 'planner.rl.policy_planner', module)
    RLPolicyOperationalPlanner(configuracion_operacional=config)
...
def test_rechaza_maximo_no_positivo(tmp_path):
    ...
    modelo.write_bytes(b'single-policy')
    manifest = tmp_path / 'rl_policies.json'
    manifest.write_text(dumps({'version': VERSION_OPERACIONAL, 'modelo': modelo.name, 'sha256_modelo': sha256(b'single-policy').hexdigest(), 'max_pedidos_rl_validado': 0}), encoding='utf-8')
