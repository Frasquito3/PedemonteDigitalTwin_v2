...
def test_ejecuta_una_sola_politica_y_registra_decision(
    monkeypatch,
    tmp_path,
):
    _preparar(
    ...
    planner = RLPolicyOperationalPlanner(
        configuracion_operacional=_config(tmp_path),
        planner_rl=planner_falso,
    )
    ...
    assert (
        planner.ultima_decision.fuente_seleccionada
        == FUENTE_RL_UNICA
    )
...
def test_fallo_de_politica_informa_error_ejecutable(
    monkeypatch,
    tmp_path,
):
    ...
    with pytest.raises(
        RuntimeError,
        match="política RL única no pudo generar",
    ):
        planner.generar_plan(_instancia(9))


def test_fuera_de_rango_tecnico_no_ejecuta_politica(
    monkeypatch,
    tmp_path,
):
    _preparar(monkeypatch)
...
def test_rechaza_metricas_invalidas(monkeypatch, tmp_path):
    _preparar(monkeypatch, tardios=-1)
    planner = RLPolicyOperationalPlanner(
        configuracion_operacional=_config(tmp_path),
        planner_rl=_PlannerFalso(_plan()),
    )

    with pytest.raises(RuntimeError, match="métricas inválidas"):
        planner.generar_plan(_instancia(8))
...
def test_rechaza_tipo_invalido_para_mascara(tmp_path):
    modelo = tmp_path / "rl_policy.zip"
    modelo.write_bytes(b"single-policy")
    manifest = tmp_path / "rl_policies.json"
    manifest.write_text(
    ...
    with pytest.raises(ValueError, match="debe ser booleano"):
        cargar_configuracion_operacional(manifest)


def test_construye_planner_con_mascara_dura(
    monkeypatch,
    tmp_path,
):
    modelo = tmp_path / "rl_policy.zip"
    modelo.write_bytes(b"policy")
    config = ConfiguracionOperacionRL(
        version=VERSION_OPERACIONAL,
        modelo=modelo,
        sha256_modelo="",
        max_pedidos_rl_validado=12,
    ...
    monkeypatch.setitem(
        sys.modules,
        "planner.rl.policy_planner",
        module,
    )

    RLPolicyOperationalPlanner(
        configuracion_operacional=config,
    )
