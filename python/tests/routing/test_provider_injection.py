...
class ProveedorViajePrueba:
    distancia_metros: float = 2_500.0
    tiempo_base_min: float = 6.0
    llamadas: list[tuple[Coordenada, Coordenada]] = field(
...
class InyeccionProveedorAlgoritmosTest(unittest.TestCase):
    ...
    def test_greedy_acepta_proveedor_inyectado(self) -> None:
        ...
        self.assertGreater(len(proveedor.llamadas), 0)
        self.assertGreaterEqual(plan.costo_estimado, 0.0)

    def test_random_acepta_proveedor_inyectado(self) -> None:
        proveedor = ProveedorViajePrueba()
    ...
    def test_ga_usa_el_mismo_proveedor_en_matriz_y_semilla_greedy(
        self,
    ) -> None:
        proveedor = ProveedorViajePrueba()
        ...
        self.assertGreaterEqual(
            len(proveedor.llamadas),
            llamadas_por_matriz * 2,
        )
        self.assertGreaterEqual(plan.costo_estimado, 0.0)

    def test_selector_inyecta_el_mismo_proveedor_en_modos_clasicos(
        self,
    ) -> None:
        proveedor = ProveedorViajePrueba()
