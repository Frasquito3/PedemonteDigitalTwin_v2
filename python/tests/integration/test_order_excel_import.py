...
def test_rechaza_id_duplicado_y_ventana_fuera_de_turno() -> None:
    with TemporaryDirectory() as temporal:
        ...
        mensajes = [
            error.mensaje
            for error in captura.value.errores
        ]

        assert any(
            "dentro del turno" in mensaje
            for mensaje in mensajes
        )
        assert any(
            "ID duplicado" in mensaje
            for mensaje in mensajes
        )


def test_parsea_horas_y_formatea() -> None:
    assert parsear_hora_a_minutos("7:30") == 450
    assert parsear_hora_a_minutos("14:05") == 845
    assert parsear_hora_a_minutos(0.5) == 720
    assert parsear_hora_a_minutos(555) == 555
    assert formatear_minutos_hora(450) == "07:30"
