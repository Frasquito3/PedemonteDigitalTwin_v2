...
class GreedyOperationalTieBreakTest(unittest.TestCase):
    def _pedido(
        ...
        return PedidoInput(
            ...
            hora_hasta_min=720,
            cliente=pedido_id,
            direccion=f"Dirección {pedido_id}",
            barrio="Prueba desempate operacional",
            observaciones="",
        )
    ...
    def _instancia(
        ...
        return InstanciaTurno(
            ...
            turno=Turno.MANANA,
            pedidos=[
                self._pedido(ids["NORTE"], COORDENADA_NORTE),
                self._pedido(ids["ESTE"], COORDENADA_ESTE),
                self._pedido(ids["CERCANA"], COORDENADA_CERCANA),
            ],
            ...
            seed_escenario=15103,
            seed_ejecucion=25103,
        )

    def _firma_semantica(
        self,
        plan,
        semantica_por_id: dict[str, str],
    ) -> tuple:
        return tuple(
            tuple(
                tuple(
                    semantica_por_id[pedido_id]
    ...
    def test_renombrar_pedidos_no_cambia_la_decision_operativa(self) -> None:
        ids_numericos = {
            "NORTE": "MULTIVIAJE-001",
            "ESTE": "MULTIVIAJE-002",
            "CERCANA": "MULTIVIAJE-003",
        }
        ids_lexicos_invertidos = {
            "NORTE": "Z-NORTE",
            "ESTE": "M-ESTE",
            "CERCANA": "A-CERCANA",
        }

        instancia_a = self._instancia(
            "GREEDY-TIEBREAK-A",
            ids_numericos,
        )
        instancia_b = self._instancia(
            "GREEDY-TIEBREAK-B",
            ids_lexicos_invertidos,
        )

        proveedor = ProveedorHaversineAjustado()
        plan_a = generar_plan_greedy(
            instancia_a,
            proveedor_viaje=proveedor,
        )
        plan_b = generar_plan_greedy(
            instancia_b,
            proveedor_viaje=proveedor,
        )
        ...
        semantica_a = {
            pedido_id: nombre
            for nombre, pedido_id in ids_numericos.items()
        }
        semantica_b = {
            pedido_id: nombre
            for nombre, pedido_id in ids_lexicos_invertidos.items()
        }

        self.assertEqual(
            self._firma_semantica(plan_a, semantica_a),
            self._firma_semantica(plan_b, semantica_b),
        )
    ...
    def test_viajes_equivalentes_se_ordenan_por_carga_operativa(self) -> None:
        ids = {
            "NORTE": "Z-NORTE",
            "ESTE": "M-ESTE",
            "CERCANA": "A-CERCANA",
        ...
        semantica = {
            pedido_id: nombre
            for nombre, pedido_id in ids.items()
        }
        firma = self._firma_semantica(plan, semantica)

        # El pedido cercano no debe asignarse primero solo porque su ID
        # empiece con "A". El ID funciona únicamente como último desempate.
        primer_pedido = firma[0][0][0]
        self.assertNotEqual("CERCANA", primer_pedido)
