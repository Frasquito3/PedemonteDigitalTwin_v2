...
def _instancia() -> InstanciaTurno:
    pedidos = [
        PedidoInput(
            pedido_id=f"P{indice:03d}",
            pedido_original_id=f"P{indice:03d}",
            numero_parte=1,
            total_partes=1,
            turno=Turno.MANANA,
            latitud=-32.84 - indice * 0.001,
            longitud=-60.71 - indice * 0.001,
...
def test_ga_acepta_semilla_rl_y_puede_excluir_greedy():
    ...
    planner = GeneticAlgorithmPlanner(
        configuracion_ga=ConfiguracionGA(
            ...
            tamano_elite=2,
            tamano_torneo=2,
            generaciones_sin_mejora_max=4,
        ),
        seed=16002,
        semillas_iniciales=(semilla,),
        incluir_semilla_greedy=False,
        fraccion_variantes_semilla=0.5,
    )
