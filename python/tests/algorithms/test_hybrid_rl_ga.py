...
from planner.algorithms.hybrid_rl_ga import FuenteResultadoHibrido, HybridRLGAPlanner, MotivoResultadoHibrido
...
class HybridRLGAPlannerTest(unittest.TestCase):
    ...
    def crear_instancia(self) -> InstanciaTurno:
        return InstanciaTurno(instancia_id='HIBRIDO-RL-GA-TEST', fecha_operacion='2026-08-27', turno=Turno.MANANA, pedidos=[PedidoInput(pedido_id='P001', pedido_original_id='P001', numero_parte=1, total_partes=1, turno=Turno.MANANA, latitud=-32.831, longitud=-60.719, unidades_capacidad=4, requiere_volcador=False, tiene_ventana_especifica=False, hora_desde_min=450, hora_hasta_min=720), PedidoInput(pedido_id='P002', pedido_original_id='P002', numero_parte=1, total_partes=1, turno=Turno.MANANA, latitud=-32.8595006, longitud=-60.702653, unidades_capacidad=4, requiere_volcador=False, tiene_ventana_especifica=False, hora_desde_min=450, hora_hasta_min=720)], lat_corralon=-32.8495006, lon_corralon=-60.722653, capacidad_camion=8, cantidad_camiones=2, hora_inicio_turno_min=450, hora_fin_objetivo_min=720, hora_fin_tolerancia_min=735, seed_escenario=15105, seed_ejecucion=1015105)

    def plan_ga_con_delta(self, instancia: InstanciaTurno, delta: float) -> PlanTurno:
        plan = generar_plan_greedy(instancia)
    ...
    def test_refina_con_ga_si_mejora_la_semilla_rl(self) -> None:
        ...
        planner = HybridRLGAPlanner(planner_rl=PlannerRLFalso(delta=0.0), generador_ga_refinado=generar_ga)
        plan = planner.generar_plan(instancia)
        decision = planner.ultima_decision
        self.assertIsNotNone(decision)
        assert decision is not None
        self.assertEqual(decision.resultado, FuenteResultadoHibrido.REFINADO_GA)
    ...
    def test_conserva_rl_si_ga_no_mejora(self) -> None:
        instancia = self.crear_instancia()
        planner = HybridRLGAPlanner(planner_rl=PlannerRLFalso(delta=0.0), generador_ga_refinado=lambda actual, _semilla: self.plan_ga_con_delta(actual, delta=1.0))
        plan = planner.generar_plan(instancia)
        decision = planner.ultima_decision
        self.assertIsNotNone(decision)
        assert decision is not None
        self.assertEqual(decision.resultado, FuenteResultadoHibrido.SEMILLA_RL)
        self.assertEqual(decision.motivo, MotivoResultadoHibrido.SEMILLA_RL_CONSERVADA)
    ...
    def test_fallo_ga_conserva_semilla_rl_y_lo_audita(self) -> None:
        ...
        planner = HybridRLGAPlanner(planner_rl=PlannerRLFalso(delta=0.0), generador_ga_refinado=ga_con_error)
        plan = planner.generar_plan(instancia)
        decision = planner.ultima_decision
        self.assertIsNotNone(decision)
        assert decision is not None
        self.assertEqual(plan.algoritmo, AlgoritmoPlanificacion.RL)
        self.assertEqual(decision.motivo, MotivoResultadoHibrido.GA_NO_EJECUTABLE)
        self.assertTrue(decision.error_ga)

    def test_fallo_rl_impide_iniciar_el_hibrido(self) -> None:
        planner = HybridRLGAPlanner(planner_rl=PlannerRLFalso(error=RuntimeError('fallo RL')))
