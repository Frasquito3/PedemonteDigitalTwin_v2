...
class PedemonteTrainingEnv(
    ...
    def __init__(
        ...
        if (
            self.semillas_fijas is not None
            and not self.semillas_fijas
        ):
            raise ValueError(
                "semillas_fijas no puede estar vacío."
            )
    ...
    def _seed_para_indice(
        self,
        indice: int,
    ) -> int:
        if indice < 0:
            raise ValueError(
                "indice no puede ser negativo."
            )
    ...
    def reset(
        ...
        *,
        ...
        seed_instancia = (
            self._seed_para_indice(
                self._indice_episodio
            )
        )
        ...
        instancia = self.generador.generar(
            seed_instancia
        )

        nuevo_env = self._crear_entorno(
            instancia
        )

        if (
            nuevo_env.action_space
            != self.action_space
        ):
            nuevo_env.close()

            raise RuntimeError(
                "El action_space cambió entre "
                "instancias de entrenamiento."
            )

        if (
            nuevo_env.observation_space
            != self.observation_space
        ):
            nuevo_env.close()

            raise RuntimeError(
                "El observation_space cambió entre "
                "instancias de entrenamiento."
            )
