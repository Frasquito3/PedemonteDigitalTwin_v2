...
class RegistroTemporalPedido:
    ...
    def llegada_tardia(self) -> bool:
        return self.tardanza_llegada_min > 1e-9
...
@dataclass(frozen=True)
class ResultadoPotencialTemporal:
    potencial: float
    penalizacion_bruta: float
    tardanza_prefijo_min: float
    espera_larga_prefijo_min: float
    pedidos_tardios_prefijo: int
    pedidos_pendientes_en_riesgo: int
    tardanza_pendiente_proyectada_min: float


def _validar_prefijo(
    ...
    if len(prefijo) != len(set(prefijo)):
        raise ValueError(
            "El prefijo temporal contiene pedidos repetidos."
        )

    desconocidos = [
        pedido_id
        for pedido_id in prefijo
        if pedido_id not in ids_validos
    ]

    if desconocidos:
        raise ValueError(
            "El prefijo temporal contiene pedidos desconocidos: "
            f"{desconocidos}."
        )


def _agrupar_viajes_prefijo(
    instancia: InstanciaTurno,
...
def analizar_prefijo_temporal(
    ...
    finales_camiones = [
        ...
        for _ in range(instancia.cantidad_camiones)
    ]

    cantidad_viajes_camion = [
        0
        for _ in range(instancia.cantidad_camiones)
    ]
    ...
    for pedido_ids in _agrupar_viajes_prefijo(
        instancia,
        pedidos_por_id,
        prefijo_tuple,
    ):
        camion_id = min(
            range(instancia.cantidad_camiones),
            key=lambda candidato: (
                finales_camiones[candidato],
                cantidad_viajes_camion[candidato],
                candidato,
            ),
        )
        ...
        unidades = sum(
            pedidos_por_id[pedido_id].unidades_capacidad
        ...
        for orden_visita, pedido_id in enumerate(
            ...
            minuto_actual += tiempo_viaje_esperado_min(
                matriz,
                nodo_actual,
                pedido_id,
                minuto_actual,
            ...
            tardanza = max(
                0.0,
                minuto_llegada - pedido.hora_hasta_min,
            )
            ...
            registros.append(
                RegistroTemporalPedido(
                    ...
                    minuto_salida_viaje=minuto_salida,
                    minuto_llegada=minuto_llegada,
                    espera_apertura_min=(
                        espera.tiempo_espera_ventana_min
                    ),
                    espera_respuesta_min=(
                        espera.tiempo_espera_respuesta_min
                    ),
                    minuto_inicio_descarga=(
                        minuto_inicio_descarga
                    ),
                    minuto_fin_descarga=minuto_fin_descarga,
                    holgura_llegada_min=(
                        pedido.hora_hasta_min
                        - minuto_llegada
                    ),
                    holgura_fin_descarga_min=(
                        pedido.hora_hasta_min
                        - minuto_fin_descarga
                    ),
                    tardanza_llegada_min=tardanza,
                )
            )
    ...
    minuto_referencia = max(
        finales_camiones,
        default=float(instancia.hora_inicio_turno_min),
    )
    ...
    espera_total = sum(
        registro.espera_apertura_min
        for registro in registros
    )
    pedidos_tardios = sum(
        1
        for registro in registros
        if registro.llegada_tardia
    )
    ...
    if not all(isfinite(valor) for valor in valores):
        raise RuntimeError(
            "El estimador temporal produjo valores no finitos."
        )

    return ResumenTemporalPrefijo(
        prefijo=prefijo_tuple,
        registros=tuple(registros),
        finales_camiones_min=tuple(finales_camiones),
        minuto_referencia=minuto_referencia,
        tardanza_total_min=tardanza_total,
        espera_apertura_total_min=espera_total,
        pedidos_tardios=pedidos_tardios,
    )


def proyectar_acciones_pendientes(
    instancia: InstanciaTurno,
    matriz: MatrizViaje,
    configuracion: ConfiguracionPlanificacion,
    prefijo: Iterable[str],
) -> dict[str, ProyeccionTemporalAccion]:
    prefijo_tuple = tuple(prefijo)
    ...
    for pedido in instancia.pedidos:
        ...
        resumen = analizar_prefijo_temporal(
            ...
            matriz,
            configuracion,
            (*prefijo_tuple, pedido.pedido_id),
        )
        ...
        if registro is None:
            raise RuntimeError(
                "No se pudo recuperar la proyección temporal de "
                f"{pedido.pedido_id}."
            )

        proyecciones[pedido.pedido_id] = (
...
def _penalizacion_registro(
    registro: RegistroTemporalPedido,
    configuracion: ConfiguracionTemporalRL,
) -> tuple[float, float]:
    tardanza_normalizada = (
        registro.tardanza_llegada_min
        / configuracion.margen_critico_min
    )

    perdida_holgura = max(
        0.0,
        configuracion.margen_critico_min
        - max(0.0, registro.holgura_llegada_min),
    ) / configuracion.margen_critico_min

    espera_larga = max(
        0.0,
        registro.espera_apertura_min
        - configuracion.umbral_espera_larga_min,
    )

    espera_larga_normalizada = (
        espera_larga
        / configuracion.umbral_espera_larga_min
    )

    penalizacion = (
        configuracion.peso_tardanza
        * tardanza_normalizada
        + configuracion.peso_margen_critico
        * perdida_holgura
        + configuracion.peso_espera_larga
        * espera_larga_normalizada
    )

    return penalizacion, espera_larga


def calcular_potencial_temporal(
    resumen: ResumenTemporalPrefijo,
    proyecciones: dict[str, ProyeccionTemporalAccion],
    configuracion: ConfiguracionTemporalRL,
) -> ResultadoPotencialTemporal:
    penalizacion = 0.0
    espera_larga_prefijo = 0.0

    for registro in resumen.registros:
        penalizacion_registro, espera_larga = (
            _penalizacion_registro(
                registro,
                configuracion,
            )
        )
        penalizacion += penalizacion_registro
        espera_larga_prefijo += espera_larga

    pendientes_en_riesgo = 0
    tardanza_pendiente = 0.0

    for proyeccion in proyecciones.values():
        registro = proyeccion.registro
        penalizacion_registro, _ = (
            _penalizacion_registro(
                registro,
                configuracion,
            )
    ...
    potencial = (
        -penalizacion
        / configuracion.escala_potencial
    )

    return ResultadoPotencialTemporal(
        potencial=potencial,
        penalizacion_bruta=penalizacion,
        tardanza_prefijo_min=(
            resumen.tardanza_total_min
        ),
        espera_larga_prefijo_min=(
            espera_larga_prefijo
        ),
        pedidos_tardios_prefijo=(
            resumen.pedidos_tardios
        ),
        pedidos_pendientes_en_riesgo=(
            pendientes_en_riesgo
        ),
        tardanza_pendiente_proyectada_min=(
            tardanza_pendiente
        ),
    )
